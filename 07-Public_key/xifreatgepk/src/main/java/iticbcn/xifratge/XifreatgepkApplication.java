...
public class XifreatgepkApplication {

	public static void main(String[] args) throws Exception {

        ClauPublica cp = new ClauPublica();

        String msg = "Missatge de prova per xifrar áéíóú àèìòù ÄËÏÖÚ";

        KeyPair parellClaus = cp.generaParellClausRSA();

        byte[] msgXifrat = cp.xifrarRSA(msg, parellClaus.getPublic());

        System.out.println("===================");
        System.out.println("Text xifrat: ");
        System.out.println(DatatypeConverter.printHexBinary(msgXifrat));

        String msgDesxifrat = cp.desxifrarRSA(msgXifrat, parellClaus.getPrivate());
        System.out.println("=================");
        ...
        SpringApplication.run(XifreatgepkApplication.class, args);
    }
}
